...
angular.module('crucio')
  .controller('AnalyzeCtrl', function ($scope, $location, API, Auth, Collection) {
    ...
    $scope.correct_answer = function(id) {
      if (!$scope.collection.questions[id]) { return false; }
      return $scope.collection.questions[id].correct_answer;
    };
    ...
    Collection.loadQuestions($scope.answeredQuestionIDList, $scope.user.user_id).then(function() {
      $scope.collection = Collection.getCollection();
